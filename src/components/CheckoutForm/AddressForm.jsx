...
const AddressForm = ({ checkoutToken, test }) => {
  ...
  const getSubdivisions = async (countryCode) => { // calls subdivision when calling useeffect
  ...
  // gets the shipping options country for a specific product.
  const getShippingOptions = async (checkoutTokenId, country, stateProvince = null) => {
    const options = await commerce.checkout.getShippingOptions(checkoutTokenId, { country, region: stateProvince });

    setShippingOptions(options);
    setShippingOption(options[0].id); // selects the first available shipping option
  };
